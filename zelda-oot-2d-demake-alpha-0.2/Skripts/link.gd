...
# HUD-Referenz (wird in _ready() initialisiert)
var hud: HUD = null
...
func _ready() -> void:
	# Warte 1 Frame, damit der HUD sich in die Gruppe "hud" eintragen kann.
	await get_tree().process_frame
	hud = get_tree().get_first_node_in_group("hud") as HUD
	if hud:
		hud.set_action_text("Nichts")
...
func _physics_process(delta: float) -> void:
	...
	_update_action_prompt(move_input)
	move_and_slide()
...
#region Item-Input (C-Buttons)
func _unhandled_input(e: InputEvent) -> void:
	# Blocken, wenn das Menü offen ist
	if ui_bus.menu_open:
		return
...
func _on_area_exited(_area: Area2D) -> void:
	...
	if is_climbing and climb_zone_count == 0:
		exit_climb_state()
	if hud:
		hud.set_action_text("Klettern" if climb_zone_count > 0 else "Nichts")
...
# --- HILFSFUNKTION NEU ---
func _update_action_prompt(move_input: Vector2) -> void:
	if hud == null:
		return
	if is_climbing or climb_zone_count > 0:
		hud.set_action_text("Klettern")
	elif move_input != Vector2.ZERO and not is_rolling:
		hud.set_action_text("Rollen")
	else:
		hud.set_action_text("")
